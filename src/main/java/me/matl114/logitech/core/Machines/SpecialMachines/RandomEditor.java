...
public class RandomEditor extends AbstractMachine implements FinalAltarCore.FinalAltarChargable, Laser.LaserChargable {
    ...
    private int getMaxEnchant() {
        if (ConfigLoader.RANDOM_EDITOR != null && ConfigLoader.RANDOM_EDITOR.contains("random_editor.max_enchant_level")) {
            return ConfigLoader.RANDOM_EDITOR.getInt("random_editor.max_enchant_level");
        }
        return 255;
    }

    private double getMaxAttribute() {
        if (ConfigLoader.RANDOM_EDITOR != null && ConfigLoader.RANDOM_EDITOR.contains("random_editor.max_attribute_value")) {
            return ConfigLoader.RANDOM_EDITOR.getDouble("random_editor.max_attribute_value");
        }
        return 1024;
    }
    ...
    public void randomEdit(ItemMeta meta, Material material) {
        ...
        if (index < registeredEnchantments.length) {
            ...
            if (level + upgrade <= getMaxEnchant()) {
            ...
            if (modifiers != null) {
                for (AttributeModifier mod : modifiers) {
                    if (VersionedAttribute.getInstance().getAttributeModifierSlot(mod) == slot
                        ...
                        if (amount >= getMaxAttribute()) {
                            break;
                        }
                        if (VersionedAttribute.getInstance()
                                .setAttributeModifierValue(mod, Math.min(mod.getAmount() + upgrade, getMaxAttribute()))) {
                            break;
                        }
                        // mod.
