...
import me.matl114.logitech.core.Machines.SpecialMachines.BoolGenerator;
...
public class RecipeSupporter {
    public static void init() {
        if (!hasInit) {
            Debug.logger("[RecipeSupporter] Initializing recipe supportor...");
            initRecipeSupportor();
            Debug.logger("[RecipeSupporter] Recipe supportor initialized");
        }
    }
    ...
    public static final List<MachineRecipe> UNUSTACKABLE_ITEM_RECIPES = new ArrayList<>() {
        {
            ...
            add(MachineRecipeUtils.shapeFrom(
                    -1,
                    Utils.recipe("BUCKET", null, null, null, null, "BUCKET", null, null),
        ...
        for (String machineId : keySets) {
            ...
            if (applyToMachine && !isGenerator) {
                if (!tryModifyStandardMachineRecipe(machine, recipes, append)) {
                    Debug.logger("WARNING: %s 不是标准机器,无法修改其配方,如需修改请在配置末尾加上 + 号".formatted(machineId));
                }
            }
        }
    }
    ...
    public static MachineRecipe transferRSCRecipes(MachineRecipe rsc) {
        ...
        try {
            ...
            try {
                ...
                if (noConsumeObj != null) {
                    // 处理 size() 方法和 getInt(int) 方法的 fastutil IntList
                    try {
    ...
    private static boolean hasRescanned = false;
    // 调试开关：只在需要排查 RSC 兼容性时打开
    private static final boolean RSC_DEBUG = false;
    ...
    private static boolean isGeoResourceMachine(SlimefunItem item) {
        ...
        // 识别本附属的地理资源矿机
        if (item == AddDepends.INFINITY_GEOQURRY || item == AddDepends.ANNIHILATION_GEOQURRY) {
            return true;
        }
        // 识别外部附属的GEO_QUARRY
        String id = item.getId();
        if (id.contains("GEO_QUARRY") || id.contains("GEOQURRY")) {
            return true;
        }
        // 识别GEOQuarry类
        if (ReflectUtils.isExtendedFrom(item.getClass(), ".GEOQuarry")) {
            return true;
        }
        return false;
    }
    ...
    public static void initRecipeSupportor() {
        ...
        Debug.logger("开始初始化配方支持器,可能需要2秒");
        Debug.logger("服务器tps较低时,可能会造成轻微卡顿");
        Debug.logger("正在加载配方...");
        Debug.logger("正在注册基础配方类型...");
        ...
        for (SlimefunItem item : Slimefun.getRegistry().getEnabledSlimefunItems()) {
            RecipeType recipeType = item.getRecipeType();
            // customRecipeType, 排除Multiblock type
        ...
        Debug.logger("完成实体掉落物加载");
        Debug.logger("正在加载配方图标...");
        // 加载配方类型图标
        ...
        Debug.logger("正在加载机器配方...");
        Debug.logger("注意: 正在加载机器配方,请勿关闭服务器");
        // 加载机器配方
        for (SlimefunItem item : Slimefun.getRegistry().getEnabledSlimefunItems()) {
            try {
                processMachineRecipes(item);
            } catch (Throwable e) {
        ...
        // 加载堆叠机器,将StackMachine和stackGenerator分开
        // 这里是机器配置加载部分
        Debug.logger("正在加载机器配置");
        // 加载配方类型
        updateStackLists();
        // 加载配置中的堆叠机器配置
        try {
    ...
    /**
     * 处理单个 SlimefunItem 的配方扫描，存入 MACHINE_RECIPELIST（如果有有效配方）
     *
     * @return true 表示成功扫描到配方并存储
     */
    private static boolean processMachineRecipes(SlimefunItem item) {
        List<MachineRecipe> recipes = new ArrayList<>();
        boolean isRsc = item.getClass().getName().contains("rykenslimefuncustomizer");
        if (isRsc && RSC_DEBUG) {
            Debug.logger("[RSC SCAN] item=" + item.getId() + " class=" + item.getClass().getName());
        }
        if (item instanceof AContainer) {
            recipes = ((AContainer) item).getMachineRecipes();
            if (isRsc && RSC_DEBUG) {
                Debug.logger("[RSC SCAN] -> instanceof AContainer, recipes=" + (recipes != null ? recipes.size() : "null"));
            }
        } else if (item instanceof AbstractTransformer atransformer) { // 抽象转换器 有特殊配方处理
            ...
            if (isRsc && RSC_DEBUG) Debug.logger("[RSC SCAN] -> resolveSpecialGenerators matched, recipes=" + recipes.size());
        } else if (isRsc && RSC_DEBUG) {
            Debug.logger("[RSC SCAN] -> NO MATCH (AContainer/CustomMaterialGenerator/AbstractTransformer/SpecialGenerators), item might be CustomGenerator");
        } else {
            boolean blst = false;
        ...
        if (!recipes.isEmpty()) {
            ...
            if (isRsc && RSC_DEBUG) Debug.logger("[RSC SCAN] -> stored, resultRecipes=" + result.size());
            return true;
        } else if (isRsc && RSC_DEBUG) {
            Debug.logger("[RSC SCAN] -> recipes EMPTY, NOT stored");
        }
        return false;
    }

    /**
     * 根据 MACHINE_RECIPELIST 的配方类型，将 item 分发到 STACKMGENERATOR_LIST 或 STACKMACHINE_LIST
     */
    private static void updateStackLists() {
    ...
    /**
     * 重扫外部插件注册的新机器（兼容后加载的附属如 RSC）
     * 在 PostSetupTasks.schedulePostRegister() 延迟调用时触发
     */
    public static void rescanMachineRecipes() {
        if (hasRescanned) {
            return; // 只跑一次，防止被循环中的机器方法反复触发
        }
        hasRescanned = true;
        int added = 0;
        for (SlimefunItem item : Slimefun.getRegistry().getEnabledSlimefunItems()) {
            if (MACHINE_RECIPELIST.containsKey(item)) {
                continue; // 已经扫过了，跳过
            }
            try {
                if (processMachineRecipes(item)) {
                    added++;
                }
            } catch (Throwable e) {
                Debug.logger("rescanMachineRecipes unexpected exception for " + item.getId() + ": " + e.getMessage());
            }
        }
        if (added > 0) {
            Debug.logger("[RSC FIX] 重扫发现 " + added + " 个新机器，更新堆叠列表...");
            updateStackLists();
        } else {
            Debug.logger("[RSC FIX] 重扫完成，无新机器需要处理");
        }
    }

    static {
        Debug.logger("正在加载配方支持器...");
        initRecipeSupportor();
    }
    ...
    public static boolean resolveSpecialGenerators(SlimefunItem item, List<MachineRecipe> recipes) {
        if (ReflectUtils.isExtendedFrom(item.getClass(), "GrowingMachine")) {
            ...
            int ticks = 1;
            try {
                Object amountObj = ReflectUtils.invokeGetRecursively(item, Settings.FIELD, "speed");
                Object materialObj = ReflectUtils.invokeGetRecursively(item, Settings.FIELD, "material");
                
                // 检查是否获取到了正确的值
                if (amountObj == null || materialObj == null) {
                    Debug.logger("Error in loading Slimefun Item %s StackMachineRecipe: Missing speed or material field"
            ...
            try {
                ...
                if (ticksPerOutput != null) {
                    ticks = ticksPerOutput - 1;
                }
            } catch (Throwable e) {
                ticks = 0;
            }
            try {
                List<MachineRecipe> geoRecipes = new ArrayList<>();
                for (SlimefunItem sfItem : Slimefun.getRegistry().getEnabledSlimefunItems()) {
                    if (sfItem.getRecipeType() == RecipeType.GEO_MINER) {
                        try {
                            Object geoResourceObj = sfItem;
                            boolean obtainable = false;
                            try {
                                obtainable = (Boolean) geoResourceObj.getClass().getMethod("isObtainableFromGEOMiner").invoke(geoResourceObj);
                            } catch (Throwable e) {
                                obtainable = true;
                            }
                            if (obtainable) {
                                Object itemObj = geoResourceObj.getClass().getMethod("getItem").invoke(geoResourceObj);
                                if (itemObj instanceof ItemStack) {
                                    geoRecipes.add(MachineRecipeUtils.mgFrom(ticks, new ItemStack[0], new ItemStack[]{((ItemStack) itemObj).clone()}));
                                }
                            }
                        } catch (Throwable e) {
                            Debug.debug("Error getting GEOResource item for " + sfItem.getId() + ": ", e.getMessage());
                        }
                    }
                }
                if (geoRecipes.isEmpty()) {
                    List<MachineRecipe> providedRecipes = PROVIDED_SHAPED_RECIPES.get(RecipeType.GEO_MINER);
                    if (providedRecipes != null && !providedRecipes.isEmpty()) {
                        for (MachineRecipe recipe : providedRecipes) {
                            geoRecipes.add(MachineRecipeUtils.mgFrom(ticks, new ItemStack[0], recipe.getOutput()));
                        }
                    } else {
                        geoRecipes.add(MachineRecipeUtils.mgFrom(ticks, new ItemStack[0], new ItemStack[0]));
                    }
                }
                recipes.addAll(geoRecipes);
            } catch (Throwable e) {
                Debug.logger("Error in loading Slimefun Item %s StackMachineRecipe: %s"
                        .formatted(item.getId(), e.getMessage()));
                return false;
